...
$idx=$_GET['id']; 
$result = mysql_query("SELECT v.vId,v.vCantidadProducto, v.vFecha,v.vIdTiket, v.vMesa, vd.vdId, vd.vd_Cantidad, vd.vd_Costo, vd.vd_Estado, p.pNombre,p.pTicket ,per.pNombres, per.pApellidos from venta v inner JOIN venta_detalle vd on vd.vId=v.vId INNER JOIN productos p on vd.vd_idProducto = p.idProducto INNER JOIN persona per on v.vIdpersona = per.pId and vd.vId='$idx' and p.pTicketera = 1 and vd.vd_Cantidad > 0", $link); 
echo '<center>***************************<br>CASA DE BARRO<br>***************************<br>TICKET BARRA</center><br>';
...
//echo '<a onclick="javascript:actualizaestadomesa(\''.$getEstadoMesaCaja['mNumero']. '\',\''.$Costo.'\',\'VISA\'); return false;" href="#"><img src="imagenes/icono/visa.png" width="70" height="40" alt="visa" /></a></div>';
  ...
  echo '<br><center>***************************<br>COCINA<br>***************************<br></center>';
  echo $fecha;  
